...
messagebox.showinfo('SMS GRATIS','Author: Henrycko')
...
def clicked():
	...
	try:
		if no.get() == '':
			messagebox.showwarning('Masukan Nomor Aktif Dengan Benar')
			T=False
		elif len(msg.get()) < 5:
			messagebox.showwarning('Pesan Error','Pesan minimal 5 karakter')
			T=False
		...
		if T == True:
			...
			if 'SMS Berhasil Dikirim' in str(sub):
				...
				label1.pack()
			else: label2=Label(root,text='[!] Gagal Mengirim ke %s'%(no.get()),fg='white',bg='black');label2.pack()
	except:
		messagebox.showerror('Error','Sepertinya terjadi kesalahan!')
	
def keluar():
	res=messagebox.askyesno("Exit","Keluar?")
	if res == True:
		print("Selesai!")
		exit()
